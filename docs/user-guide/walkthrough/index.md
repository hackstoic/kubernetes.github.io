...
在这个例子里， 我们创建了一个带有命名过的存储卷的Redis pod ，挂载的存储卷定义了要挂载的卷的路径。  
1. 定义一个卷:
...
2. 在容器定义里定义一个卷的挂载Define a volume mount within a container definition:
...
带有持久化存储卷的 Redis pod 的定义的例子 ([pod-redis.yaml](/docs/user-guide/walkthrough/pod-redis.yaml)):

{% include code.html language="yaml" file="pod-redis.yaml" ghlink="/docs/user-guide/walkthrough/pod-redis.yaml" %}

备注:

- 在 `volumeMounts` 中`name` 指向的是  `volumes`中 `name`.
- 在 `volumeMounts`的 `mountPath`  是在容器里要挂载的卷的目录的路径。

##### 卷的类型

- **EmptyDir**:  会创建一个目录， 这个目录会一直存在直到这个Pod不在这个Node上运行， 但容器启动失败和重启， 这个目录还是会存在。 
- **HostPath**:  挂载 node 的文件系统里已经存在的目录(比如. `/var/logs`).

查看文档 [volumes](/docs/concepts/storage/volumes/) 获取更多细节。 
...
##  多 containers
备注:
下面的这些例子在语法上保证正确， 但是里面的一些镜像( 比如 kubernetes/git-monitor) 并不存在。 我们正在努力把这些制作成例子。 


然而，如果您经常想要有两个工作在一起的容器。 比如在这个例子中，会有一个 web 服务端程序， 还有一个辅助的任务进程， 用来拉取镜像仓库进行更新 :
...
注意到我们在这里增加里一个卷。 在这种情况下， 卷会同时被两个容器挂载。因为不需要往这个目录写入数据， 所以在web server的场景下， 这个目录被标记为 `只读` 。 

最后我们要引入给`git-monitor` 的环境遍量， 这些环境变量允许我们队我们要跟踪的特定的git 仓库进行参数化。 
...
继续学习 [Kubernetes 201 课程](/docs/user-guide/walkthrough/k8s201) 或者 完成一个完整的应用例子 [guestbook example](https://github.com/kubernetes/kubernetes/tree/{{page.githubbranch}}/examples/guestbook/)
